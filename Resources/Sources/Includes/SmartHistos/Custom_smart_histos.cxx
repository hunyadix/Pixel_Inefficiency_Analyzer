...
void Custom_smart_histos::AddNewPostfix(std::unique_ptr<Postfix>& postfix_p)
{
	...
	postfix_name_to_postfix_size_t_map.insert(std::make_pair(postfix_name, 0));

	const size_t& postfix_size_t_to_read = postfix_name_to_postfix_size_t_map.at(postfix_name);
...
void Custom_smart_histos::UpdatePostfix(const std::string& name)
{
	...
	{
		size_t& postfix_to_update = postfix_name_to_postfix_size_t_map.at(name);
