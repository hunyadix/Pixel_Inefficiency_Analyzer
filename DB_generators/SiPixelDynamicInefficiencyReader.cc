...
void SiPixelDynamicInefficiencyReader::analyze( const edm::Event& e, const edm::EventSetup& iSetup){
  ...
  unsigned int match=0,mismatch=0,pu_match=0;
  ...
  std::cerr<<"debug: PU_DET = "<< pu_det << std::endl;
  max_matches++;
  ...
  if (pu_match != max_matches ) std::cout<<"ERROR! "<< max_matches - pu_match <<" pu_factors mismatched to config file pu_factors! Please change config and/or DB content!"<<std::endl;
}